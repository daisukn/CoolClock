...
void Mesh::SetAnimID(int id, PlayMode m, float start, float end)
{
    if(prevAnimID != id)
    {
        playTime = 0.0f;
...
void Mesh::BoneTransform(float deltaTime, std::vector<Matrix4> &Transforms)
{
    ...
    if (bScrapMotion)
    {
        ...
        if (mode == PLAY_ONCE)
        {
            if (TimeInTicks + startFrame > endFrame)
            {
        ...
        float AnimationTime = fmod(TimeInTicks, endFrame - startFrame);// (float)pScene->mAnimations[animID]->mDuration);
        
        ReadNodeHeirarchy(AnimationTime + startFrame, pScene->mRootNode, identity);
