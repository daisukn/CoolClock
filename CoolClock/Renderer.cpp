...
bool Renderer::Initialize(std::string title, float scWidth, float scHeight)
{
    ...
    //ウインドウ生成
    lpWnd = SDL_CreateWindow(strTitle.c_str(),
    ...
    /*
    lpWnd = SDL_CreateShapedWindow(strTitle.c_str(),
                           SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                           640, 480,
                           SDL_WINDOW_BORDERLESS);
     */
...
void Renderer::Draw()
{
    glClearColor(0.38f, 0.634f, 0.458f, 1.0f);
...
bool Renderer::LoadShaders()
{
    ...
    viewMatrix = Matrix4::CreateLookAt(Vector3::Zero, Vector3::NegUnitZ, Vector3::UnitY);
...
//    dirLight.DiffuseColor = Vector3(0.6f, 0.3f, 0.1f);
    ...
    shader->SetVectorUniform("uFoginfo.color", Vector3(0.3f, 0.3f, 0.3f) );
...
// 背景用スプライトコンポーネントの登録
void Renderer::AddBackground(BackgroundComponent* bg)
{
    // DrawOrderを探して 自分より優先度の高いものの次を見つける
    int myDrawOrder = bg->GetDrawOrder();
    auto iter = bgComps.begin();
    for (;iter != bgComps.end(); ++iter)
    {
    ...
    // 見つけた箇所に挿入
    bgComps.insert(iter, bg);
}

// 背景スプライト削除
void Renderer::RemoveBackground(BackgroundComponent* bg)
{
    auto iter = std::find(bgComps.begin(), bgComps.end(), bg);
    bgComps.erase(iter);
}
